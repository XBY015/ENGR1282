...
DigitalEncoder leftEncoder(FEHIO::Pin4);
DigitalEncoder rightEncoder(FEHIO::Pin5);

//Sensor declarations
AnalogInputPin optosensorLeft(FEHIO::Pin0);
AnalogInputPin optosensorMiddle(FEHIO::Pin1);
AnalogInputPin optosensorRight(FEHIO::Pin2);
...
const float MIDDLE_OPTOSENSOR_THRESHOLD = 3; //threshold value for middle optosensor on the line (black line will have a value above this threshold, white background will have a value below this threshold)
...
void followLineToIntersection(int percent) 
{
   ...
   //combine the three sensor readings into a 3 bit integer representing the current state
    int currentState = ((leftValue  > LEFT_OPTOSENSOR_THRESHOLD   ? 1 : 0) << 2) // returns 1 if on line, 0 if off line
                     | ((middleValue> MIDDLE_OPTOSENSOR_THRESHOLD ? 1 : 0) << 1)
                     | ((rightValue > RIGHT_OPTOSENSOR_THRESHOLD  ? 1 : 0) << 0); 

    while(currentState != 0&&currentState != 7&&currentState != 3){ //keep following the line until hit an intersection
        switch (currentState)
        {
            case 0: //off the line, only middle sensor is above threshold, stop motors
                leftMotor.Stop();
                rightMotor.Stop();
                break;
            ...
            case 2: //on the line, all three sensors are above threshold, go straight
                ...
                rightMotor.SetPercent(percent);
                break;
            case 7: //at an intersection, all three sensors are above threshold, stop motors and break out of loop
                leftMotor.Stop();
                rightMotor.Stop();
                break;
            case 3: //at a T intersection, middle and right sensors are above threshold, stop motors and break out of loop
                leftMotor.Stop();
                rightMotor.Stop();
                break;
        }
